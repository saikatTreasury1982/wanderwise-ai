...
/**
 * Delete auto-generated expenses for a module whose source item no longer exists.
 * keepSourceIds = the source_ids present in the current collection run.
 */
async function pruneRemovedExpenses(
  tx: Transaction,
  tripId: number,
  sourceModule: string,
  keepSourceIds: number[]
): Promise<void> {
  const existing = await queryTx<{ expense_id: number; source_id: number }>(
    tx,
    `SELECT expense_id, source_id FROM expenses WHERE trip_id = ? AND source_module = ?`,
    [tripId, sourceModule]
  );
  ...
  for (const row of existing) {
    if (!keep.has(row.source_id)) {
      // source gone → remove splits then expense (cascade removes its actuals)
      await queryTx(tx, `DELETE FROM expense_splits WHERE expense_id = ?`, [row.expense_id]);
      await queryTx(tx, `DELETE FROM expenses WHERE expense_id = ?`, [row.expense_id]);
    }
  }
}
...
async function validateCurrency(tx: Transaction, currency: string): Promise<string> {
  ...
  if (validCurrency.length === 0) {
    // Auto-create the missing currency
    await queryTx(
      tx,
...
/**
 * Upsert an expense (match by trip+module+source_id) and recreate its splits.
 * Preserves expense_id when the item already exists → linked actuals survive.
 */
async function upsertExpenseWithSplits(
  tx: Transaction,
  tripId: number,
  categoryId: number,
  description: string,
  ...
  // does this source item already have an expense?
  const existing = await queryTx<{ expense_id: number }>(
    tx,
    `SELECT expense_id FROM expenses WHERE trip_id = ? AND source_module = ? AND source_id = ?`,
    [tripId, sourceModule, sourceId]
  );

  let expenseId: number;

  if (existing.length > 0) {
    // UPDATE in place — keep expense_id (actuals stay linked)
    expenseId = existing[0].expense_id;
    await queryTx(
      tx,
      `UPDATE expenses
       SET category_id = ?, expense_description = ?, estimated_amount = ?, expense_currency = ?
       WHERE expense_id = ?`,
      [categoryId, description, amount, expenseCurrency, expenseId]
    );
  } else {
    // INSERT new
    await queryTx(
  ...
  // recreate splits (derived data, no user input — safe to rebuild)
  await queryTx(tx, `DELETE FROM expense_splits WHERE expense_id = ?`, [expenseId]);
...
async function collectAndSaveFlightCosts(
  ...
  const bookings = await queryTx<{
    ...
    `SELECT
     ...
     WHERE fb.trip_id = ? AND fb.status = 'confirmed'`,
    [tripId]
  );

  for (const b of bookings) {
    const amount = b.total_paid ?? b.base_fare ?? 0;
    if (amount <= 0) continue;

    const legs = await queryTx<{ departure_airport_code: string | null; arrival_airport_code: string | null }>(
      tx,
      `SELECT departure_airport_code, arrival_airport_code
       FROM flight_booking_legs
       WHERE booking_id = ?
       ORDER BY leg_order`,
      [b.booking_id]
    );
    ...
    if (legs.length > 0) {
      routeDescription = `${legs[0].departure_airport_code ?? ''} → ${legs[legs.length - 1].arrival_airport_code ?? ''}`;
    }
    const description = routeDescription || b.notes || `Flight #${b.booking_id}`;

    await upsertExpenseWithSplits(
      tx, tripId, categoryId, description, amount, b.currency_code || 'USD',
      'flights', b.booking_id, costSharers
    );
    keepIds.push(b.booking_id);
  ...
  await pruneRemovedExpenses(tx, tripId, 'flights', keepIds);
  return items;
}
...
async function collectAndSaveAccommodationCosts(
  ...
  if (statuses.length === 0) {
    // no statuses selected → nothing should remain
    await pruneRemovedExpenses(tx, tripId, 'accommodations', keepIds);
    return items;
  }
  ...
  for (const a of accommodations) {
    ...
    await upsertExpenseWithSplits(
      tx, tripId, categoryId, description, amount, a.currency_code || 'USD',
      'accommodations', a.accommodation_option_id, costSharers
    );
    keepIds.push(a.accommodation_option_id);
...
async function collectAndSaveItineraryCosts(
  ...
  for (const a of activities) {
    ...
    await upsertExpenseWithSplits(
      tx, tripId, categoryId, description, amount, a.currency_code,
      'itinerary', a.activity_id, costSharers
    );
    keepIds.push(a.activity_id);
  ...
  for (const c of categories) {
    ...
    const sourceId = c.category_id + 100000;

    await upsertExpenseWithSplits(
      tx, tripId, categoryId, description, amount, c.currency_code,
      'itinerary', sourceId, costSharers
    );
    keepIds.push(sourceId);
  ...
  await pruneRemovedExpenses(tx, tripId, 'itinerary', keepIds);
  return items;
}

/**
 * Collect ad-hoc expense costs and save to expenses (within transaction)
 */
async function collectAndSaveAdhocCosts(
  tx: Transaction,
  tripId: number,
  categoryId: number,
  costSharers: TravelerInfo[]
  ...
  for (const e of expenses) {
    const assignedTravelers = await queryTx<{ traveler_id: number }>(
      tx,
      `SELECT traveler_id 
       FROM adhoc_expense_travelers 
       WHERE adhoc_expense_id = ?`,
      [e.adhoc_expense_id]
    );

    const expenseCostSharers = costSharers.filter(cs =>
      assignedTravelers.some(at => at.traveler_id === cs.traveler_id)
    );

    if (expenseCostSharers.length === 0) continue;

    const description = e.category
      ? `${e.expense_name} (${e.category})`
      : e.expense_name;

    await upsertExpenseWithSplits(
      tx, tripId, categoryId, description, e.amount, e.currency_code,
      'adhoc_expenses', e.adhoc_expense_id, expenseCostSharers
    );
    keepIds.push(e.adhoc_expense_id);

    items.push({
      id: e.adhoc_expense_id,
      module: 'adhoc_expenses',
      description,
      amount: e.amount,
  ...
  await pruneRemovedExpenses(tx, tripId, 'adhoc_expenses', keepIds);
  return items;
}
...
export async function collectCosts(options: CostCollectorOptions): Promise<CostForecastReport> {
  ...
  const { flightCosts, accommodationCosts, itineraryCosts, adhocCosts } = await withTransaction(async (tx) => {
    ...
    const adhocCategoryId = await getOrCreateCategory(tx, 'Ad-hoc Expenses');
    ...
    const adhocCosts = await collectAndSaveAdhocCosts(tx, tripId, adhocCategoryId, costSharers);

    return { flightCosts, accommodationCosts, itineraryCosts, adhocCosts };
  });

  const allItems = [...flightCosts, ...accommodationCosts, ...itineraryCosts, ...adhocCosts];
  ...
  const moduleBreakdown: ModuleBreakdown[] = [
    ...
    {
      module: 'adhoc_expenses',
      total: adhocCosts.reduce((sum, i) => sum + (i.converted_amount || i.amount), 0),
      currency_code: baseCurrency,
      items_count: adhocCosts.length,
      items: adhocCosts,
    },
  ];
