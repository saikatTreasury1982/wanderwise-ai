...
async function collectAndSaveFlightCosts(
  ...
  // Actual confirmed bookings only
  ...
  for (const b of bookings) {
    ...
    // route from booking legs (first departure → last arrival)
...
async function collectAndSaveAdhocCosts(
  ...
  // Delete existing auto-generated adhoc expenses
  await deleteAutoGeneratedExpenses(tx, tripId, 'adhoc_expenses');

  const items: CostLineItem[] = [];

  // Get active ad-hoc expenses with their assigned travelers
  ...
  for (const e of expenses) {
    // Get travelers assigned to this expense
