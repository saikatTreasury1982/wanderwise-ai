...
export async function GET(request: Request, { params }: { params: Promise<{ tripId: string; rangeId: string; categoryId: string; activityId: string }> }) {
  try {
...
export async function POST(request: Request, { params }: { params: Promise<{ tripId: string; rangeId: string; categoryId: string; activityId: string }> }) {
  try {
    const { activityId } = await params;
    const cookieStore = await cookies();
    const sessionToken = cookieStore.get('session')?.value;
    ...
    if (!link_url) return NextResponse.json({ error: 'Link URL is required' }, { status: 400 });
